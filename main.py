...
# After action ends the script is back to the beginning of the cycle. The
# user chooses action again
...
while action_number != 12:
    ...
    if action_number == 1:
        ...
        phone_to_find = str(
            input('Ввдеите номер телефона для поиска: ')).strip()
        try:
        ...
        middlename_to_find = str(
            input('Введите отчество для поиска: ')).strip()
        try:
